...
class Reliable:
    def __init__(self, hport=10000):
        ...
        self.bytesInFly = 0
        self.rwnd = MaxBDP
        # Use 4*SMSS instead of 3*SMSS since SMSS=1024<=1095 (RFC5681)
        self.cwnd = 4*PayloadSize
    ...
    def connect(self, ip='127.0.0.1', rport=7090, n=None):
        ...
        while self.status != CONNECTED:
            ...
            self.sendto(Segment.pack(seqNum, 0, 0, 0, 1, 0,
                        ReliableImpl.checksum(temp), b''))
        ...
        while self.reli.status != CLOSED:
            ...
            for skt in readable:
                ...
                if self.reli.status == CONNECTED:
                    if seg.ack and not seg.syn and not seg.fin:
                        self.reli.bytesInFly -= self.reliImpl.recvAck(seg)
            ...
            for skt in writable:
                ...
                if payload is None:
                    ...
                    self.reli.bytesInFly += self.reliImpl.sendData(
                        payload, False)
